...
def gen_preset_random_map(random, width, height, num_players=2):
    players = [1] * (num_players - 1) + [2]
    target = [3]
    map = players + target + (width * height - (1 + num_players)) * [0]
    random.shuffle(map)
    return np.array(map).reshape(width, height).astype(np.uint8)
...
def get_string_map(map, tiles):
    ...
    # TODO findout why this dirty workaround is neccessary. 
    # print(map)
    # if (map == 5).any():# or (map == 5).any():
    #    map = nmmo_to_rl2(map)
    # print(map)
